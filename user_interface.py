...
def login():
    ...
    if st.button("Login"):
        if st.session_state["password"] == st.secrets[st.session_state["username"]]:
...
if selected_db:
    ...
    if st.session_state["username"] == st.secrets["allow"]:
        collection_names = db.list_collection_names()
        selected_collection = st.sidebar.selectbox("Select Collection", collection_names)
    else:
        selected_collection = st.session_state["password"]
